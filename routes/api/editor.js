...
//  上傳圖片
router.post('/img', api_check_login, Cache.modifiedtCache, uploadImg)

//  為Blog的blogImg建立另一筆blogImgAlt數據
router.post('/blogImgAlt', api_check_login, Cache.modifiedtCache, async(ctx, next) => {
    let { blogImg_id, blog_id } = ctx.request.body
    ctx.body = await addBlogImgAlt(blogImg_id, blog_id)
})

//  修改blogImgAlt數據
router.patch('/blogImgAlt', api_check_login, Cache.modifiedtCache, async(ctx, next) => {
...
router.patch('/initImgs', api_check_login, Cache.modifiedtCache, async(ctx, next) => {
    const { id: user_id } = ctx.session.user
