...
window.addEventListener("load", async () => {
  ...
  async function initMain({ blog }) {
    ...
    let $span_content_count = $(`#${PAGE_BLOG_EDIT.ID.BLOG_HTML_STRING_COUNT}`);
    /* ------------------------------------------------------------------------------------------ */
    ...
    function init_editor() {
      ...
      async function customUpload(img, insertFn) {
        ...
        if (!exist) {
          ...
          let api = `${PAGE_BLOG_EDIT.API.CREATE_IMG}?hash=${hash}&name=${name}&ext=${ext}&blog_id=${$$pageData.blog.id}`;
      ...
      async function _() {
        ...
        if (!error) {
          ...
          $span_content_count.text(`文章內容不能為空`).addClass("text-danger");
          //  提示html不可為空
        } else if (error["diff"]) {
          $$payload._del(KEY);
