const {
  DEFAULT: {
    COMMENT: { CHECK_IS_DELETED, SORT_BY },
  },
} = require("../config");
...
function removeDeletedComment(list) {
  ...
  function getListOfNeedToDelete(
    commentList,
    _opts = { p_id: undefined, map: undefined }
  ) {
    let opts;
    ...
    function statisticsByMap(comment, opts) {
      ...
      //  所屬留言串總數 + 當前留言串的留言數
      reply.forEach(({ id }) => p_idReplyTotal.add(id));
      //  重新設置所屬留言串總數
      mapTotal.set(commentId, p_idReplyTotal);

      //  所屬留言串所包含的 已刪除留言數量
      let p_idDelCount = mapDel.get(commentId);
      if (!p_id && !p_idDelCount) {
        //  若此次為所屬留言串總數的根留言 且 p_idDelCount統計資料未定義，則設定為0
        p_idDelCount = new Set();
      } else if (p_id) {
        //  否則 p_idDelCount + 1，並重新設定所屬留言串的已刪除留言數量
