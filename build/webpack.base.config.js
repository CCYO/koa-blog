...
const entry = ((filepathList) => {
  ...
  filepathList.forEach((filepath) => {
    ...
    const filename = list[list.length - 1].replace(/\.js/g, "");
    // 如果是开发环境，才需要引入 hot module
    // res[key] = filepath
    res[filename] = SERVER.ENV.isDev
      ? // filepath
